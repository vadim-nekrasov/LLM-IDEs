![img.png](img.png)# Get started with Claude Code hooks - Claude Code Doc
...
-   **Automatic formatting**: Run 
    ...
    gofmt
    ```
     on .go files, etc. after every file edit.
...
/hooks
```
 [slash command](https://code.claude.com/docs/en/slash-commands) and select the 
```
PreToolUse
```
 hook event. 
...
PreToolUse
```
 hooks run before tool calls and can block them while providing Claude feedback on what to do differently.
...
+ Add new matcher…
```
 to run your hook only on Bash tool calls. Type 
```
Bash
```
 for the matcher.
...
For storage location, select 
```
User settings
```
 since you’re logging to your home directory. This hook will then apply to all projects, not just your current project. Then press 
```
Esc
```
 until you return to the REPL. Your hook is now registered.
...
Ask Claude to run a simple command like 
```
ls
```
