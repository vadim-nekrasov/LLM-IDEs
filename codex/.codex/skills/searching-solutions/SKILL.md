...
2. **Architecture & Idiomaticity**: Scalable, pattern-consistent, fits stack. Design Principles respected (SOLID, GRASP, DRY, KISS, YAGNI). Not overengineered.
...
Before brainstorming, consider using web search if it is enabled.
If web search is unavailable, ask the user for context or sources.
