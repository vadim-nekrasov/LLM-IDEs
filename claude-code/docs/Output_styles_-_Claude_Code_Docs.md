...
-   Custom output styles exclude instructions for coding (such as verifying code with tests), unless 
    ```
...
-   Run 
    ...
    /output-style
    ```
     to access a menu and select your output style (this can also be accessed from the 
    ```
    /config
    ```
     menu)
...
You can save these files at the user level (
```
...
description
```
 | Description of the output style. Used only in the UI of 
```
/output-style
```
 | None |
...
```
 edit Claude Code’s default system prompt. CLAUDE.md adds the contents as a user message _following_ Claude Code’s default system prompt. 
```
--append-system-prompt
```
