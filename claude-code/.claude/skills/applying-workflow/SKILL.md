...
3. **Phase 2 — Three Lenses Analysis.** See `../_shared/three-lenses.md`. For multi-file refactors or architecture-keyword prompts also apply `../_shared/solid-audit.md` (apply / skip decision lives in the rubric). Confidence < 85 % on critical logic → `patterns/confidence-check.md`.
4. **Phase 3 — Implementation.** Sync comments with logic; follow the Maintainer Lens comments policy — `../_shared/three-lenses.md#c-maintainer-lens--future--team`. No zombie code. Add logs first when bug cause is unclear; never patch blindly.
